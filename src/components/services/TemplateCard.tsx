    ...
    >
      ...
      <div className="mt-3 flex flex-wrap gap-1">
        {template.modules.slice(0, 3).map((m) => (
          <span key={m} className="inline-flex h-5 px-1.5 items-center rounded text-[10px] bg-muted text-muted-foreground">
            {m}
          </span>
        ))}
      </div>
      ...
      <div className="mt-4 pt-3 border-t border-border flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1">
          <Button variant="ghost" size="sm" className="h-7 px-2 text-[11px]" onClick={onPreview}>
            <Eye className="h-3 w-3 mr-1" /> Preview
          </Button>
          <Button variant="ghost" size="sm" className="h-7 px-2 text-[11px]" onClick={onDetails}>
