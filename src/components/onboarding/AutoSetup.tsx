...
import { tradeTemplate } from "@/data/serviceTemplates";
    ...
    if (currentIndex >= setupSteps.length) {
      setTimeout(() => {
        const newService: ServiceItem = {
          id: state.selectedTemplateId || `application-${Date.now()}`,
          name: state.serviceName,
          templateId: state.selectedTemplateId,
          status: "draft",
          customModules: state.customModules.length > 0 ? state.customModules : tradeTemplate.modules,
          isPublished: false,
          isLive: false,
